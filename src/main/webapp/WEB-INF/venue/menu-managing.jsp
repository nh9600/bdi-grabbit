...
<script>
    ...
    <input type="hidden" value="${venue.viCode}" id="currenVenueCode">
	<div class="container">
...
function goPage(page,searchParam){
	$.ajax({
		url:'/menus?viNum=' + $('#currenVenueNum').val() + '&viCode=' + $('#currenVenueCode').val(),
		method:'GET',
		...
		success:function(res){
			...
			for(lists of res.list){
				...
				html += '<td onclick="goView('+ lists.miNum +')">'+ lists.mcCode +'</td>';
				html += '<td onclick="goView('+ lists.miNum +')">'+ lists.miName +'</td>';
				html += '<td onclick="goView('+ lists.miNum +')">'+ lists.miPrice +'</td>';
				if(lists.miImg==null){
					html += '<td><img src="test.jpg">사진 없음</td>'; //test.jpg는 없는 사진입니다.
				}else if(lists.miImg.includes('https')){
					html += '<td><img src="'+ lists.miImg + '" width="100" height="100" alt="' + lists.miImg + '"></td>';
				}else{
					html += '<td><img src="'+ imgPath+lists.miImg + '" width="100" height="100" alt="' + lists.miImg + '"></td>';
				}
				//html += '<td><img src="' + imgPath + lists.miImg + '" width="100" height="100" alt="' + lists.miImgFile + '"></td>';
				html += '</tr>'
			}
			$('#tBody').html(html);
