			...
			<div class="row ">
				<div class="col-lg-4">
					<div class="service-img">
	                    ...
	                    <input i-data-menu="miImgFile" id="miImgFile" type="file" name="miImgFile" class="form-control">
					</div>
				</div>
...
$(document).ready(function(){
	$.ajax({
	    ...
	    success:function(res){
	    	...
	    	//사진
	    	$('#miImgFile').hide(); 
	   		if(list.miImg.includes('https')){
	   			$('#miImg').attr('src',list.miImg);
	   		}else{
	   			$('#miImg').attr('src',imgPath+list.miImg);
	   		}
	    	//$('#miImg').attr('src', imgPath + list.miImg);
	    	//메뉴 가격,카테고리
	    	...
	    	for(var idx = 0; idx < tds.length; idx++){
	    		...
	    		td.innerHTML = '<input class="form-control" readonly="readonly" value="'+list[key]+'">';
	    	}
	    }
	});
...
$('.updateMenuInfo').on('click',function(){
	//메뉴 이름
	const h = document.querySelector('h1[data-Menu]');
	h.innerHTML = '<input i-data-menu="miName" name="miName" value="'+ list.miName +'">';
	//메뉴 소개말
	const p = document.querySelector('p[data-Menu]');
	p.innerHTML = '<input i-data-menu="miIntro" name="miIntro" class="form-control" value="'+ list.miIntro +'">';
	var tds = document.querySelectorAll('td[data-Menu]');
	for (var i = 0; i < tds.length; i++) {
	var td = tds[i];
	var id = td.getAttribute('data-Menu');
	$('#miImgFile').show(); //사진 선택
	if(id=='mcCode'){
		td.innerHTML = '<select i-data-menu="mcCode" name="mcCode"><option value=0>선택 </option><option value=1>001 커피 </option><option value=2>002 차 </option><option value=3>003 에이드 </option><option value=4>004 스무디 </option><option value=5>005 블렌디드 음료 </option><option value=6>006 베이커리 </option></select>';
	}else if(id=='moCode'){
		td.innerHTML = '<select i-data-menu="moCode" name="moCode"><option value=0>선택 </option><option value=1>001 핫/아이스,얼음량,시럽,샷,휘핑 </option><option value=2>002 핫/아이스,우유량,시럽,휘핑</option><option value=3>003 핫/아이스,얼음량</option><option value=4>004 얼음량</option><option value=5>005 휘핑</option><option value=6>006 옵션 없음</option>';
	}else if(id=='miPrice'){
		td.innerHTML = '<input i-data-menu="miPrice" class="form-control" name="miPrice" value="'+list[id]+'">';
	}else if(id=='miImg'){
		td.innerHTML = '<input i-data-menu="miImg" class="form-control" name="miImg"  value="'+list[id]+'">';
	  }
	}
    ...
    var objs = document.querySelectorAll('select[i-data-menu],input[i-data-menu],textarea[name]')
	...
	if($('[name=miIntro]').val().trim().length < 1){
		alert('메뉴 소개말을 입력해주세요.');
	}
	if($('[name=mcCode]').val() == 0){
		alert('카테고리를 선택해주세요.');
		return false;
	}
	if($('[name=moCode]').val() == 0){
		alert('메뉴옵션 코드를 선택해주세요.')
		return false;
	}
